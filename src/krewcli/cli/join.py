...
async def _run_gateway(
    settings, cookbook_id, agent_id_prefix, working_dir,
...
def register_join_commands(main: click.Group) -> None:
    ...
    @click.pass_context
    def join(ctx, cookbook, port, agent_id, workdir, agents, max_concurrent,
        ...
        if any([provider, model, endpoint, framework, orchestrator]):
            raise click.UsageError(
                ...
                "Use 'krewcli daemon start --cookbook CB' instead."
            )
    ...
    @click.pass_context
    def start(ctx, cookbook, agent, agent_id, port, workdir):
        """[Legacy] Start an agent. Use 'daemon start' instead."""
        ctx.invoke(join, cookbook=cookbook, agent=agent,
